...
def load_data():
    ...
    df = df.merge(df_fwd[['Date', 'Forward_1M']], on="Date", how="inner")
...
def monthly_hedging(df):
    """
    对冲逻辑：每月建仓，次月最早一个交易日平仓
    """
    # 1. 提取每月的第一条记录作为建仓日
    ...
    # 4. 定义函数：找到下一个自然月的第一个实际交易日
    def get_next_month_first_trading_date(current_date):
        ...
        next_month_dates = [
            ...
            if (date.year == next_year) and (date.month == next_month)
        ]

        # 返回下个月的第一个交易日（如果存在）
        return next_month_dates[0] if len(next_month_dates) > 0 else pd.NaT

    # 5. 为每个合约找“次月实际可交易的第一天”
    monthly_data['Next_Month_Date'] = monthly_data['Date'].apply(get_next_month_first_trading_date)

    # 6. 合并 End Spot（平仓价）
    next_month_spots = df[['Date', 'USDCAD', 'USD_Net_Exposure']].rename(columns={
        'Date': 'Next_Month_Date',
        'USDCAD': 'End_Spot',
        'USD_Net_Exposure' : 'Exposure'
    })
    results = monthly_data.merge(next_month_spots, on='Next_Month_Date', how='left')

    # 7. 标准化列名
    results = results.rename(columns={
        'Date': 'Contract_Date',
        'USDCAD': 'Start_Spot',
        'Forward_1M': 'Contract_Forward'  # 若你在原始 monthly_data 里有 forward 率
    })

    # 8. 如果没有 Exposure，则添加默认值
    if 'Exposure' not in results.columns:
        results['Exposure'] = 1_000_000

    # 9. 计算盈亏
    results['Partial_Hedged'] = (results['End_Spot'] - results['Contract_Forward'] ) * results['Exposure']
    ...
    print('对冲波动率:', hedged_std)
    print('未对冲波动率:', unhedged_std)

    # 10. 返回标准输出列
    output_cols = [
        'Contract_ID', 'Contract_Date', 'Next_Month_Date',
...
if __name__ == "__main__":
    ...
    # 计算对冲结果（默认100%对冲）
    out = monthly_hedging(df)  # ← 改这里，接收字典
